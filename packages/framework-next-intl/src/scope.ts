...
/**
 * next-intl scopes: each useTranslations('A.B') / getTranslations('A.B') call's
 * containing function body. Note: the argument populates ScopeRange.keyPrefix,
 * NOT namespace — see ADR 0001 §3.
 */
export function getScopeRangeNextIntl(
  ...
  traverse(parsed.ast, {
    CallExpression(path: NodePath<t.CallExpression>) {
      ...
      if (name !== 'useTranslations' && name !== 'getTranslations') return
      ...
      if (arg && arg.type === 'StringLiteral') {
        out.push({ range, keyPrefix: arg.value })
      } else if (arg) {
